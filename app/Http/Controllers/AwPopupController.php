...
class AwPopupController extends Controller
{
...
// storing registration for e-newsletter in database
    public function store (Request $request){
        ...
        else
        {
            ...
            $popup->country='AW REG';

            if ($popup->save())
            {
                $data =['email'=>$email, 'token' => $token];
                Log::info("Request cycle with Queues started -aw popup");
                $this->dispatch(new AwSendPopupConf($data));
                Log::info("Request cycle with Queues started -aw finished");
                print "";
            }
            else
    ...
    public function confirm (Request $request){
        ...
        return view('aw.confirm',compact('mainMenuOutput','trail1','trail2'));
    }
